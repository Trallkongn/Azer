...
uniform float u_MetallicFactor;   // 默认 1.0
uniform float u_RoughnessFactor;  // 默认 1.0
uniform float u_AOFactor;         // 默认 1.0
uniform float u_Exposure;         // 用于调节最终亮度
...
void main()
{
    ...
    float ao = texture(aoMap, TexCoords).r * u_AOFactor;
    ...
    color = vec3(1.0) - exp(-color * u_Exposure);  // 曝光公式
