...
namespace Azer {
	...
	void OpenGLRendererAPI::DrawLine(const Azer::Ref<VertexArray> vertexArray, uint32_t count)
	{
	...
	void OpenGLRendererAPI::DrawPoints(const Azer::Ref<VertexArray> vertexArray, uint32_t count)
	{
	...
	void OpenGLRendererAPI::DeepTest(bool statu)
	{
		if (statu)
			glEnable(GL_DEPTH_TEST);
		else
